...
public class RateLimiterUtil {
    ...
    public void init() {
        // 这个方法会在类初始化后立即执行
        this.tokenCountMap.put("queryFlow", 1.0);
        this.tokenCountMap.put("queryCardActiveTime", 1.0);
        this.tokenCountMap.put("queryFlowHis", 1.0);
        this.tokenCountMap.put("queryCardStatus", 1.0);
        this.tokenCountMap.put("changeCardStatus", 1.0);
        this.tokenCountMap.put("changeCardStatusFlexible", 1.0);
        this.tokenCountMap.put("queryRealNameStatus", 1.0);
        this.tokenCountMap.put("queryAPNInfo", 1.0);
    ...
    public RateLimiter getGlobalRateLimiter(String cd_userName) {
        return globalRateLimiters.computeIfAbsent(cd_userName, userName -> RateLimiter.create(10.0));
    }

    // 特定方法限流器，每个账号每秒最多1个请求
