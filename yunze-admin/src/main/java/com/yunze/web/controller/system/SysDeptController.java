...
public class SysDeptController extends MyBaseController
{
     ...
     */
    @PreAuthorize("@ss.hasPermi('system:dept:list')")
    @PostMapping(value = "/tblist", produces = { "application/json;charset=UTF-8" })
    public String list(@RequestBody String Pstr)
    {
        ...
        try {
            ...
            return MyRetunSuccess(deptService.selMap(Parammap),null);
        }catch (Exception e){
            String ip = IpUtils.getIpAddr(ServletUtils.getRequest());
            logger.error("<br/> /system/dept/tblist <br/> Pstr = " + Pstr + " <br/> ip =  " + ip + " <br/> ",e.getCause().toString());
        }
        return Myerr("获取部门列表 表格版 操作失败！");
    }
