...
import com.yunze.system.service.yunze.bulk.IYzExecutionTaskService;
...
public class YzExecutionTaskController extends MyBaseController {
    ...
    public String list(@RequestBody String Pstr) {
        ...
        try {
            ...
            logger.error("<br/> yunze:ExecutionTask:list  <br/> Pstr = " + Pstr + " <br/> ip =  " + ip + " <br/> ",
                    e.getCause().toString());
        }
        ...
        try {
            ...
            if (Parammap.get("path") != null && Parammap.get("token") != null) {
                ...
                try {
                    ...
                    logger.error("<br/> yunze:ExecutionTask:download token获取用户操作失败 <br/> Pstr = " + Pstr
                            + " <br/> ip =  " + ip + " <br/> ", e.getCause().toString());
                }
                if (loginUser != null) {
                    SysUser User = loginUser.getUser();
                    logger.info("<br/> " + " 公司 " + User.getDept().getDeptName() + "  用户名 " + User.getUserId() + " "
                            + User.getUserName() + " <br/> Pstr = " + Pstr + " <br/> ip =  " + ip + " <br/> ");
                    // 获取当前文件下载 地址
                    File file2 = new File("");
                    ...
                    if (!downloadFile.exists()) {
                        logger.error("下载文件不存在: {}", path);
                        response.setStatus(HttpServletResponse.SC_NOT_FOUND);
                        response.getWriter().write("文件不存在: " + path);
                        return;
                    }
            ...
            logger.error("<br/> yunze:ExecutionTask:download IOException <br/> Pstr = " + Pstr + " <br/> ip =  " + ip
                    + " <br/> ", ex.getCause().toString());
        } catch (Exception e) {
            logger.error("<br/> yunze:ExecutionTask:download Exception <br/> Pstr = " + Pstr + " <br/> ip =  " + ip
                    + " <br/> ", e.getCause().toString());
        }
    }
    ...
    public void downloadConversion(@RequestParam String Pstr, HttpServletResponse response,
            HttpServletRequest request) {
        {
            ...
            try {
                ...
                logger.error("<br/> yunze:ExecutionTask:downloadConversion  <br/> Pstr = " + Pstr + " <br/> ip =  " + ip
                        + " <br/> ", e.getCause().toString());
            }
        }
    }
