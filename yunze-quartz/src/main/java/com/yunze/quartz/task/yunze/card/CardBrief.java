...
/**
 * 定时任务 yz_card_brief 表初始化 任务（从yz_card_info 表 分批次查询数据插入到该表中）
 */
@Component("cardBrief")
public class CardBrief {
    ...
    public void BatchByBatchAdd() {
        //1.创建路由 绑定 生产队列 发送消息
        //导卡 路由队列
        String polling_queueName = "admin_BatchByBatchAdd_queue";
        String polling_routingKey = "admin.BatchByBatchAdd.queue";
        ...
        try {
            ...
            start_type.put("Masage", "备份 执行 BatchByBatchAdd_queue ");//启动类型
